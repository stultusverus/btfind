...
mod tests {
    ...
    fn test_search_torrents() {
        let store = test_db();
        store
            .upsert_torrent([1u8; 20], Some("ubuntu iso".into()), 1_000_000, 1, "dht")
            .unwrap();
        store
            .upsert_torrent([2u8; 20], Some("debian iso".into()), 2_000_000, 1, "dht")
            .unwrap();
        store
            .upsert_torrent([3u8; 20], Some("fedora iso".into()), 3_000_000, 1, "dht")
            .unwrap();
